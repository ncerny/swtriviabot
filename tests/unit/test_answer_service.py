...
def test_submit_answer_updates_existing_answer(mock_storage):
    ...
    existing_session.add_or_update_answer(
        Answer(
            user_id=user_id,
            username=username,
            text="First answer",
...
class TestArchiveSession:
    ...
    def test_archives_session_with_answers(self, mock_storage):
        ...
        session.add_or_update_answer(
            Answer(user_id="user1", username="Alice", text="4", timestamp=now)
        )
    ...
    def test_handles_missing_question_text(self, mock_storage):
        ...
        session.add_or_update_answer(
            Answer(user_id="user1", username="Alice", text="answer", timestamp=now)
        )
        mock_storage.load_session.return_value = session
